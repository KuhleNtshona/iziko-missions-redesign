...
const nav = ['Home', 'About', 'Our Work', 'Magnify Church', 'Meet the Team', 'Stories', 'Get Involved', 'Contact'];
const pillars = [
  ...
  return (
    <main>
      <header className={`site-header ${scrolled ? 'is-scrolled' : ''}`}>
        ...
        <nav className="desktop-nav" aria-label="Main navigation">{nav.map((item) => <a key={item} href={item === 'About' ? './about/' : item === 'Meet the Team' ? './team/' : item === 'Get Involved' ? './get-involved/' : `#${item.toLowerCase().replaceAll(' ', '-')}`}>{item}</a>)}</nav>
        <a className="give-button" href="https://www.zeffy.com/en-US/donation-form/support-the-nogadas" target="_blank" rel="noopener noreferrer">Give <ArrowUpRight size={15} /></a>
        <button className="menu-button" aria-label="Toggle menu" aria-expanded={open} onClick={() => setOpen(!open)}>{open ? <X /> : <Menu />}</button>
        {open && <nav className="mobile-nav" aria-label="Mobile navigation">{nav.map((item) => <a key={item} onClick={() => setOpen(false)} href={item === 'About' ? './about/' : item === 'Meet the Team' ? './team/' : item === 'Get Involved' ? './get-involved/' : `#${item.toLowerCase().replaceAll(' ', '-')}`}>{item}</a>)}<a className="mobile-give" href="https://www.zeffy.com/en-US/donation-form/support-the-nogadas" target="_blank" rel="noopener noreferrer">Give</a></nav>}
      </header>
      ...
      <section className="hero" id="home">
        ...
        <div className="hero-content"><p className="eyebrow"><span /> Cape Town · South Africa</p><h1>Christ proclaimed.<br />Leaders equipped.<br /><em>Churches planted.</em></h1><p className="hero-copy">We equip local leaders, plant gospel-centered churches, and serve vulnerable communities across South Africa.</p><div className="hero-actions"><a className="button button-primary" href="#about">Our mission <ArrowDown size={17} /></a><a className="button button-ghost" href="./get-involved/">Partner with us <ArrowUpRight size={17} /></a></div></div>
      ...
      <section className="magnify" id="magnify-church">
        ...
        <div className="magnify-copy reveal"><div className="section-index">03 — Featured church plant</div><p className="eyebrow-dark">Cape Town, South Africa</p><h2>Meet<br /><em>Magnify Church.</em></h2><p className="body-large">A new gospel-centered church being planted in Cape Town to magnify Jesus, make disciples, and multiply faithful communities.</p><div className="magnify-stats"><div><b>Jesus</b><span>Magnified</span></div><div><b>Disciples</b><span>Made</span></div><div><b>Churches</b><span>Multiplied</span></div></div><a className="button button-dark" href="https://magnifychurch.co.za" target="_blank" rel="noopener noreferrer">Explore Magnify <ArrowUpRight size={17} /></a></div>
      </section>

      <section className="impact" id="stories">
        <div className="impact-glow" aria-hidden="true" /><div className="section-index inverted reveal">04 — Stories of impact</div><div className="quote-wrap reveal"><Quote size={36} strokeWidth={1} /><blockquote>“The goal is not simply to start programmes. It is to see local churches become places of <em>hope, truth, and lasting change.</em>”</blockquote><div className="quote-by"><span className="avatar">SN</span><p><b>Sive Nogada</b><small>Founder · Iziko Missions</small></p><a className="story-button" href="https://www.youtube.com/watch?v=A5KUkcj6_tE" target="_blank" rel="noopener noreferrer" aria-label="Watch Pastor Sive Nogada's story on YouTube"><Play size={15} fill="currentColor" /> Watch the story</a></div></div>
      ...
      <section className="involve section-light" id="get-involved">
        <div className="involve-heading reveal"><div className="section-index">05 — Get involved</div><h2>There is a place<br /><em>for you in this story.</em></h2><p>Mission is never a solo endeavour. Join us in serving churches in South Africa’s townships and rural areas.</p></div>
        <div className="ways-grid">{ways.map(({ icon: Icon, ...way }) => <a className="way-card reveal" id={way.title === 'Give' ? 'give' : undefined} href={way.title === 'Give' ? 'https://www.zeffy.com/en-US/donation-form/support-the-nogadas' : './get-involved/'} target={way.title === 'Give' ? '_blank' : undefined} rel={way.title === 'Give' ? 'noopener noreferrer' : undefined} key={way.title}><span>{way.number}</span><Icon size={28} strokeWidth={1.35} /><div><h3>{way.title}</h3><p>{way.text}</p></div><ArrowUpRight className="way-arrow" /></a>)}</div>
      </section>

      <section className="newsletter" id="contact">
        <div className="newsletter-copy reveal"><p className="eyebrow"><span /> Stay connected</p><h2>Stories from<br /><em>the field.</em></h2><p>Receive thoughtful updates, prayer needs, and stories of what God is doing through local churches.</p><p className="contact-note">We’d love to hear from you. Contact us at <a href="mailto:info@izikomission.org">info@izikomission.org</a>.</p></div>
      ...
      <footer><div className="footer-top"><a className="brand footer-brand" href="#home"><span className="brand-mark" aria-hidden="true"><i /><i /><i /></span><span>IZIKO <b>MISSIONS</b></span></a><p>Christ proclaimed. Leaders equipped. Churches planted.</p><div className="mailing"><a href="mailto:info@izikomission.org">info@izikomission.org</a><address><b>Iziko Missions South Africa</b><br/>539 W. Commerce St. #5455<br/>Dallas, TX 75208</address><small>Make checks payable to Iziko Missions South Africa.</small></div></div><div className="nonprofit">Iziko Missions South Africa is a registered 501(c)(3) nonprofit organization. Federal Tax ID: 39-3246865.</div><div className="footer-bottom"><span>© 2026 Iziko Missions</span><span>Cape Town · South Africa</span><div><a href="./about/">About</a><a href="./team/">Meet the Team</a><a href="https://www.zeffy.com/en-US/donation-form/support-the-nogadas" target="_blank" rel="noopener noreferrer">Give</a><a href="#contact">Contact</a></div></div></footer>
    </main>
  );
}
